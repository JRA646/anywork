...
function Info({ label, value }: { label: string; value: string }) {
 ...
 + Number(form.startingPrice).toLocaleString() : 'Quote'}</strong></div></div></section>
        <div className="drawerActions"><button type="button" className="buttonSecondary" onClick={() => setShowForm(false)}>Cancel</button><button type="submit" className="buttonPrimary" disabled={saving}>{saving ? 'Saving…' : editingId ? 'Update service' : 'Save service'}</button></div>
      </form>
    </div>}
  </div>
...
function Info({ label, value }: { label: string; value: string }) {
 ...
 + Number(form.startingPrice).toLocaleString() : 'Quote'}</strong></div></div></section>        <div className="drawerActions"><button type="button" className="buttonSecondary" onClick={() => setShowForm(false)}>Cancel</button><button type="submit" className="buttonPrimary" disabled={saving}>{saving ? 'Saving…' : editingId ? 'Update service' : 'Save service'}</button></div>
      </form>
    </div>}
  </div>
}
