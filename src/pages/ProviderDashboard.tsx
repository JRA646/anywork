...
function ProviderJobs({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  useEffect(() => {
    Promise.all([getCurrentUserId(), listProviderRequests()])
      .then(([id, rows]) => {
        setProviderId(id)
  ...
  return (
    <div className="workspaceDashboard providerHub">
      ...
      <div className="providerStatusTabs">
        {filters.map((item) => <button key={item} className={filter === item ? 'active' : ''} onClick={() => setFilter(item)}><span>{item}</span><b>{item === 'All' ? requests.length : filtered.filter(() => true).length}</b></button>)}
      </div>
      <div className="providerJobsList">
        {loading ? <EmptyPanel title="Loading jobs" description="Checking your confirmed work." /> : filtered.length ? filtered.map((request) => (
          <article className="providerJobCard" key={request.id}>
            ...
            <div className="providerJobValue"><span>Job value</span><strong>{request.budget !== null ? '$' + Number(request.budget).toLocaleString() : 'TBD'}</strong></div>
            <StatusBadge status={request.status} />
