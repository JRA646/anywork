...
export function PublicHome({ services, providers, onNavigate, onQuote }: { services: Service[]; providers: Provider[]; onNavigate: (path: string) => void; onQuote: (serviceId?: string) => void }) {
  return <div>
    <section className="heroModern"><div className="container heroGrid"><div className="heroCopy">
      ...
      <div className="heroSearch" onClick={() => onNavigate('/services')}><Search size={19} /><span>What do you need help with?</span><button onClick={(e) => { e.stopPropagation(); onNavigate('/services') }}>Search</button></div>
    ...
    </div><div className="heroFeatureCard">
      <span className="heroCardLabel">HOW ANYWORK WORKS</span>
      <div className="heroCardLine"><b>01</b><span>Tell us what you need</span></div><div className="heroCardLine"><b>02</b><span>Compare providers</span></div><div className="heroCardLine"><b>03</b><span>Approve your quote</span></div><div className="heroCardLine"><b>04</b><span>Track it to done</span></div>
      <button onClick={() => onQuote()}>Start a request <ArrowRight size={16} /></button>
    </div></div></section>
    <section className="sectionModern"><div className="container"><SectionHeading eyebrow="POPULAR SERVICES" title="Whatever needs doing." action="Browse all" onClick={() => onNavigate('/services')} /><div className="serviceGridModern">{services.map((service) => <ServiceCard key={service.id} service={service} onClick={() => onQuote(service.id)} />)}</div></div></section>
    <section className="sectionModern softSection"><div className="container trustGrid"><div><span className="eyebrow">WHY ANYWORK</span><h2>A better way to hire for everyday work.</h2><p className="bodyLead">One place for discovery, quotes, scheduling, messages, photos and invoices. No scattered chats. No guessing what happens next.</p></div><div className="trustTiles"><TrustTile icon={<ShieldCheck />} title="Verified providers" text="Identity, profile and service information stay visible." /><TrustTile icon={<Star />} title="Real reviews" text="See ratings and completed work before choosing." /><TrustTile icon={<CheckCircle2 />} title="Clear status" text="Every request follows one simple lifecycle." /><TrustTile icon={<Sparkles />} title="Flexible jobs" text="Use Custom when the job does not fit one category." /></div></div></section>
    <section className="sectionModern"><div className="container"><SectionHeading eyebrow="TRUSTED PROVIDERS" title="People behind the work." action="Explore services" onClick={() => onNavigate('/services')} /><div className="providerGridModern">{providers.slice(0, 3).map((provider) => <ProviderCard key={provider.id} provider={provider} onClick={() => onNavigate('/providers/' + provider.id)} />)}</div></div></section>
    <section className="sectionModern projectSection"><div className="container"><span className="eyebrow">RECENT PROJECTS</span><h2>From small fixes to full fit-outs.</h2><div className="projectGridModern"><Project title="Commercial signage" category="INSTALL" meta="Retail · Parramatta" cls="imageOne" /><Project title="Custom office furniture" category="BUILD" meta="Commercial · North Sydney" cls="imageTwo" /><Project title="Event banner setup" category="PRINT" meta="Events · Alexandria" cls="imageThree" /></div></div></section>
    <section className="ctaModern"><div className="container ctaInner"><div><span className="eyebrow">START WITH THE JOB</span><h2>Not sure which provider you need?</h2><p>Send the details. ANYwork helps route the request to the right service and providers.</p></div><button className="buttonPrimary light" onClick={() => onQuote()}>Request a service <ArrowRight size={17} /></button></div></section>
  </div>
}

function SectionHeading({ eyebrow, title, action, onClick }: { eyebrow: string; title: string; action: string; onClick: () => void }) {
  return <div className="sectionHeading"><div><span className="eyebrow">{eyebrow}</span><h2>{title}</h2></div><button className="textLink" onClick={onClick}>{action}<ArrowRight size={15} /></button></div>
}
...
function Project({ title, category, meta, cls }: { title: string; category: string; meta: string; cls: string }) {
  return <div className={'projectCard ' + cls}><span>{category}</span><strong>{title}</strong><small>{meta}</small></div>
}